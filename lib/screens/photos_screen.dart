...
class _PhotosScreenState extends State<PhotosScreen> {
  ...
  void _showStitchFrames() {
    if (_selectedStitchPhotos.length != 2) return;
    
    // Sort chronologically older to newer
    final sorted = List<Map<String, dynamic>>.from(_selectedStitchPhotos);
    sorted.sort((a, b) {
      final da = DateTime.tryParse(a['taken_at'] ?? '') ?? DateTime.now();
      final db = DateTime.tryParse(b['taken_at'] ?? '') ?? DateTime.now();
      return da.compareTo(db);
    });

    final p1 = sorted[0];
    final p2 = sorted[1];
    final diff = (DateTime.tryParse(p2['taken_at'] ?? '') ?? DateTime.now()).difference(DateTime.tryParse(p1['taken_at'] ?? '') ?? DateTime.now()).inDays.abs();
    ...
    showDialog(
      ...
      builder: (ctx) => Scaffold(
        ...
        appBar: AppBar(
          ...
          elevation: 0,
          leading: IconButton(
            icon: const Icon(Icons.close, color: ApexColors.t1),
            onPressed: () {
              Haptics.vibrate(HapticsType.light);
              Navigator.pop(ctx);
              setState(() {
          ...
          title: Text('$diff days apart', style: GoogleFonts.inter(fontWeight: FontWeight.w800, fontSize: 16, color: ApexColors.t1)),
        ),
        body: Row(
          children: [
            Expanded(
              child: Column(
                children: [
                  Expanded(
                    child: ClipRect(
                      child: _buildImage(p1['photo_data']),
                    ),
                  ),
                  Container(
                    ...
                    child: Column(
                      children: [
                        Text('BEFORE', style: TextStyle(color: ApexColors.t3, fontSize: 10, fontWeight: FontWeight.w800)),
                        const SizedBox(height: 4),
                        Text(_formatDate(p1['taken_at']), style: GoogleFonts.inter(fontSize: 14, color: ApexColors.t1, fontWeight: FontWeight.w700)),
                      ],
                    ),
                  ),
                ],
            ...
            Container(width: 2, color: ApexColors.bg),
            Expanded(
              child: Column(
                children: [
                  Expanded(
                    child: ClipRect(
                      child: _buildImage(p2['photo_data']),
                    ),
                  ),
                  Container(
                    width: double.infinity,
                    ...
                    child: Column(
                      children: [
                        Text('AFTER', style: TextStyle(color: ApexColors.accentSoft, fontSize: 10, fontWeight: FontWeight.w800)),
                        const SizedBox(height: 4),
                        Text(_formatDate(p2['taken_at']), style: GoogleFonts.inter(fontSize: 14, color: ApexColors.t1, fontWeight: FontWeight.w700)),
                      ],
                    ),
                  ),
                ],
